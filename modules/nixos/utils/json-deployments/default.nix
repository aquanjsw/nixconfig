...
let
  runtimeDirectory = "json-deployments";
...
in
{
  options.rag.utils.json-deployments = lib.mkOption {
  ...
  config.systemd.services = builtins.listToAttrs (
    lib.forEach (lib.attrsToList config.rag.utils.json-deployments) (deployment: {
