...
}:
{
  ...
  options.tunnel.client.mihomo.settings = lib.mkOption {
    ...
    default =
      ...
      in
      {
        ...
        secret = {
          _secret = secrets.controller-secret.path;
        };
  ...
  config = {
    ...
    age.secrets = {
      lan-auth.file = config.paths.secrets + "/lan-auth.age";
      controller-secret.file = config.paths.secrets + "/controller-secret.age";
    };
